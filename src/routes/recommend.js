...
router.get('/history/:googleId', async (req, res) => {
  try {
    ...
    console.log(`[Recommend] 히스토리 조회 요청: googleId=${googleId}`);
    
    // google_sub로 모든 추천 가져오기
    ...
    if (error) {
      console.error('[Supabase] 추천 히스토리 조회 오류:', error);
      return res.status(500).json({ 
        ok: false, 
        message: '추천 히스토리 조회 중 오류가 발생했습니다.',
        error: error.message 
      });
    }
    ...
    recommendations.forEach(rec => {
      ...
      const utcDate = new Date(rec.created_at);
      const kstDate = new Date(utcDate.getTime() + (9 * 60 * 60 * 1000));
      const date = kstDate.toISOString().split('T')[0]; // YYYY-MM-DD (KST)
      ...
      groupedByDate[date].tracks.push({
        id: rec.id,
        name: rec.song_title,
    ...
    console.log(`[Recommend] ✅ ${result.length}개 날짜의 추천 히스토리 반환`);
    
    res.json({ 
      ok: true, 
      data: result,
      total_dates: result.length,
      total_tracks: recommendations.length
    });
    
  } catch (error) {
    console.error('[Recommend] 히스토리 조회 오류:', error);
    res.status(500).json({ 
      ok: false, 
      message: '서버 오류가 발생했습니다.',
      error: error.message 
...
// 특정 날짜의 추천 조회
router.get('/by-date/:googleId/:date', async (req, res) => {
  try {
    const { googleId, date } = req.params;
    ...
    if (error) {
      console.error('[Supabase] 날짜별 추천 조회 오류:', error);
      return res.status(500).json({ 
        ok: false, 
    ...
    console.error('[Recommend] 날짜별 조회 오류:', error);
    res.status(500).json({ 
      ok: false, 
      message: '서버 오류가 발생했습니다.',
      error: error.message 
...
router.delete('/tracks', async (req, res) => {
  try {
    ...
    console.log(`[Recommend] 삭제 요청: ${ids.length}개 트랙`);
    ...
    if (error) {
      console.error('[Supabase] 트랙 삭제 오류:', error);
      return res.status(500).json({
        ok: false,
        message: '트랙 삭제 중 오류가 발생했습니다.',
        error: error.message
      });
    }
    
    console.log(`[Recommend] ✅ ${ids.length}개 트랙 삭제 완료`);
    
    res.json({
      ok: true,
      message: `${ids.length}개의 트랙이 삭제되었습니다.`,
      deleted_count: ids.length
    });
    
  } catch (error) {
    console.error('[Recommend] 삭제 오류:', error);
    res.status(500).json({
      ok: false,
      message: '서버 오류가 발생했습니다.',
      error: error.message
