...
router.get('/callback', async (req, res) => {
  ...
  if (error) {
    ...
    return res.redirect(`${FRONTEND_URL}/chat?spotify_error=` + error);
  }

  if (!code) {
    return res.redirect(`${FRONTEND_URL}/chat?spotify_error=no_code`);
  }
  ...
  try {
    ...
    res.redirect(`${FRONTEND_URL}/chat?spotify_token=${access_token}`);

  } catch (error) {
    console.error('[Spotify] Token exchange failed:', error.response?.data || error.message);
    res.redirect(`${FRONTEND_URL}/chat?spotify_error=token_failed`);
  }
});
...
router.get('/token', async (req, res) => {
  try {
    ...
    if (!tokenStore.access_token) {
      console.warn('[Spotify] No token in store. User needs to login first.');
